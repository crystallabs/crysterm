...
#
# Captures land beside each demo as   examples/features/<demo>-capture.gif
# (still demos as <demo>-capture.png). Override the directory with OUT=...
#
# Normally you don't run this directly — tools/manage-examples.cr drives it,
# owning the output naming and freshness checks and calling this only to render
# stale captures. It still works standalone for a manual refresh.
#
...
for demo in "${DEMOS[@]}"; do
  ...
  if [ "$demo" = "glyph_modes" ]; then
    ...
    echo "   building ..."
    crystal build "$SRC/glyph_mode.cr" -o "$BUILD/glyph_mode" 2>&1 | sed 's/^/   /' || {
      echo "   build FAILED, skipping"; continue; }
    for m in "${GLYPH_MODES[@]}"; do
      GLYPH_MODE="$m" python3 "$RECORDER" \
        --out "$OUT/glyph_mode-capture-$m.png" \
  ...
  if [ "$demo" = "overlay" ]; then
    ...
    crystal build "$SRC/overlay_image.cr" -o "$BUILD/overlay_image" 2>&1 | sed 's/^/   /' || {
      echo "   build FAILED, skipping"; continue; }
    ...
    if [ -n "$gw" ]; then
      ...
      python3 - "$OUT/png_image-capture.png" /tmp/_overlay_grab.png "$OUT/overlay_image-capture.png" <<'PY'
import sys
