...
module WidgetExamples
  ...
  # Newest modification time across the whole crysterm library (`src/**/*.cr`),
  # computed once per run. A capture depends on its program *and* the library it
  # links, so a toolkit edit (the usual reason a capture goes stale) makes every
  # output older than this and thus eligible for regeneration in the default run.
  @@newest_lib_mtime : Time? = nil

  def self.newest_lib_mtime : Time
    @@newest_lib_mtime ||= Dir.glob(File.join(ROOT, "src", "**", "*.cr"))
      .map { |f| File.info(f).modification_time }.max? || Time.unix(0)
  end

  # Whether *dest* is up to date and may be skipped. `--force` always regenerates;
  # a missing output is always (re)made. Otherwise the output is **stale** — and
  # regenerated by the default run — when its program source *src* or the library
  # is newer than it.
  def self.skip_output?(dest : String, force : Bool, src : String? = nil) : Bool
    return false if force || !File.exists?(dest)
    dest_m = File.info(dest).modification_time
    newest = newest_lib_mtime
    if src && File.exists?(src) && (sm = File.info(src).modification_time) > newest
      newest = sm
    end
    return false if newest > dest_m # a source changed after this output → stale
    puts "skip   #{relative_to_root(dest)} (up to date; --force to overwrite)"
    true
  end
  ...
  def self.process(widgets : Array(Item), opts : Options)
    ...
    widgets.each do |w|
      ...
      progs.each do |path|
        ...
        if opts.build
          bin = bin_for(path)
          if skip_output?(bin, opts.force, path)
            skipped += 1
          else
            builds << {w, path, bin}
          end
        ...
        if want_shot
          d = shot_for(path)
          skip_output?(d, opts.force, path) ? (skipped += 1) : (env["CRYSTERM_SHOT"] = d; dests << d)
        end
        if want_dump
          d = dump_for(path)
          skip_output?(d, opts.force, path) ? (skipped += 1) : (env["CRYSTERM_DUMP"] = d; dests << d)
        end
        if want_anim
          d = anim_for(path, opts.duration)
          if skip_output?(d, opts.force, path)
            skipped += 1
          else
            env["CRYSTERM_ANIM"] = d
  ...
  def self.process_tests(opts : Options)
    ...
    progs.each do |prog|
      ...
      if want_shot
        d = shot_for(prog)
        skip_output?(d, opts.force, prog) ? (skipped += 1) : (env["CRYSTERM_SHOT"] = d; dests << d)
      end
      if want_dump
        d = dump_for(prog)
        skip_output?(d, opts.force, prog) ? (skipped += 1) : (env["CRYSTERM_DUMP"] = d; dests << d)
      end
      if want_anim
        d = anim_for(prog, opts.duration)
        if skip_output?(d, opts.force, prog)
          skipped += 1
        else
          env["CRYSTERM_ANIM"] = d
          env["CRYSTERM_ANIM_SECS"] = opts.duration.to_s
