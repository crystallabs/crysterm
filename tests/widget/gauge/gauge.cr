...
# Run it:     crystal run tests/widget/gauge/gauge.cr
require "../example"
...
Crysterm::WidgetExample.run("Gauge",
  script: ->(d : Crysterm::WidgetExample::Driver) {
    ...
    [65, 75, 85, 95, 85, 75, 65].each do |v|
      d.act(dwell: 0.4) { |s| s.children.each { |c| c.value = v if c.is_a?(CW::Gauge) } }
    end
  }) do |window|
  window.stylesheet = "Gauge { border: solid; color: #7aa2f7; }"
  g = CW::Gauge.new parent: window, top: "center", left: "center", width: 40, height: 3
  g.value = 65
end
