...
tab = CT::Border.new
tab.corners = CT::Border::Corners.new(tl: CT::Border::Corner::Rounded, tr: CT::Border::Corner::Rounded)

# Each entry: {style options beyond fg/bg, 1-3 label lines}.
boxes = [
  # Row 1 — line borders: presets and new axis combinations.
  {CT::Style.new(border: CT::Border.new(type: :rounded)), # the arc-corner classic
   "type: :rounded"},
  {CT::Style.new(border: CT::Border.new(pattern: :double)), # the double-line family
   "pattern: :double"},
  {CT::Style.new(border: CT::Border.new(ratio: :full)), # ratio > 1/2 = heavy line
   "ratio: :full\n(heavy)"},
  {CT::Style.new(border: CT::Border.new(pattern: :dashed, corners: :rounded)), # composable axes
   "dashed + rounded"},
  # Row 2 — sub-cell media: block ink and braille dots.
  {CT::Style.new(border: CT::Border.new(type: :outer, ratio: :half)), # rim-flush block ink
   "block :outer\nratio: :half"},
  {CT::Style.new(border: CT::Border.new(type: :inner, ratio: :half)), # content-hugging, transparent ground
   "block :inner\n(floating ring)"},
  {CT::Style.new(border: CT::Border.new(type: :braille)), # dot ring, union corners
   "type: :braille"},
  {CT::Style.new(border: CT::Border.new(type: :outer, ratio: :thin, corner_ratio: :half)), # corner beads
   "corner beads\n(thin + :half)"},
  # Row 3 — composed looks: light-driven relief and shadows.
  {CT::Style.new(border: CT::Border.new(type: :dotted), look: :beveled), # the weight bevel, automatic
   "look: :beveled"},
  {CT::Style.new(border: true, look: :elevated), # raised shading + auto shadow
   "look: :elevated"},
  {CT::Style.new(border: tab), # border-radius: 8px 8px 0 0
   "tab corners\n(tl/tr rounded)"},
  {CT::Style.new(border: CT::Border.new(type: :braille, ratio: :half, corner_ratio: :full)), # braille beads
   "braille + beads"},
]
