...
s = CT::Window.new title: "Styling 6"
...
row1 = [
  {CT::Border.new(align: :outer, left: 2, top: 2, right: 2, bottom: 2), # rim ring, band ground inside
   "align: :outer"},
  {CT::Border.new(align: :center, left: 2, top: 2, right: 2, bottom: 2), # the classic repeat
   "align: :center"},
  {CT::Border.new(align: :inner, left: 2, top: 2, right: 2, bottom: 2), # content-hugging ring
   "align: :inner"},
  {CT::Border.new(type: :block, pattern: :double, ratio: :full, left: 3, top: 3, right: 3, bottom: 3), # rim + content rings
   "2-ring"},
]
row1.each_with_index do |(border, label), i|
  ...
  CW::Box.new \
    ...
    style: CT::Style.new(fg: "white", bg: bg, border: border)
end

# Row 2: block patterns and beads.
row2 = [
  {CT::Border.new(type: :block, pattern: :dotted, ratio: :full), # 1 ink : 1 ground cell
   "block :dotted\nratio: :full"},
  {CT::Border.new(type: :block, pattern: :dashed, ratio: :full), # 2 ink : 1 ground
   "block :dashed\nratio: :full"},
  {CT::Border.new(type: :block, pattern: :dotted, ratio: :half), # gaps at any thickness
   "block :dotted\nratio: :half"},
  {CT::Border.new(type: :outer, ratio: :thin, corner_ratio: :full), # full-block corner mounts
   "corner_ratio:\n:full (beads)"},
]
...
SEPARATORS.each_with_index do |(label, rules), i|
  ...
  panel = CW::Box.new \
    ...
    content: "{center}#{label}{/center}", parse_tags: true,
    style: CT::Style.new(fg: "white", bg: bg)
  rules.each_with_index do |axes, j|
    CW::HLine.new parent: panel, top: 1 + j, left: 1, width: 16,
      type: axes[:type], pattern: axes[:pattern], ratio: axes[:ratio],
      style: CT::Style.new(fg: fg, bg: bg)
  end
end
