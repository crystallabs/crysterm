...
s = CT::Window.new title: "Concurrent rendering"
...
CW::Box.new \
  ...
  parse_tags: true,
  style: CT::Style.new(fg: "white", bg: "#202840")
...
5.times do |i|
  ...
  pb = CW::ProgressBar.new \
    parent: s,
    top: 2 + i, left: 2, width: 46, height: 1,
    style: CT::Style.new(fg: colors[i], bg: 0x303030,
      indicator: CT::Style.new(fg: colors[i]))
  step = i + 1
...
spin1 = CW::Loading.new \
  parent: s, top: 2, left: 52, width: 24, height: 3,
  content: "Worker A", style: CT::Style.new(fg: "cyan", border: true)
spin2 = CW::Loading.new \
  parent: s, top: 6, left: 52, width: 24, height: 3,
  content: "Worker B", style: CT::Style.new(fg: "magenta", border: true)
spin1.start
spin2.start
...
marker = CW::Box.new \
  ...
  style: CT::Style.new(fg: "black", bg: "yellow")
pos = 0.0
