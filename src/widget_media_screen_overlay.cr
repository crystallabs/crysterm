...
module Crysterm
  class Widget
    ...
    module Media::ScreenOverlay
      ...
      # Scratch `RenderedGeometry` reused by both `#invalidate_old_position` (the
      # `PreRender` listener) and `#overlay_rendered` (the `Rendered`
      # listener) to avoid a heap `RenderedGeometry` allocation every window render
      # (`coords` with no `into:` allocates fresh; see
