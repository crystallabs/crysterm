module Crysterm
  ...
  class Widget
    ...
    # render. Call whenever something selector-relevant changes: the tree shape,
    # a widget's classes/id, or an intrinsic attribute. A no-op while the widget
    ...
    def to_html(io : IO, structural : Bool = false) : Nil
      ...
      # parser rewrites type selectors to class selectors), so the tag is
      # cosmetic. `#css_tag` is a hyphenated custom element to avoid HTML5's
      # special parsing of real names like `table`/`select`, which would
      # foster-parent or drop the emitted children.
      tag = css_tag
      io << '<' << tag
      io << " data-uid=\"" << uid << '"'
      if id = css_id
      ...
      # (e.g. `Form:focus Button`, lowered to `.state-focused`) match the live
      # tree. Streamed directly (no `join`): this runs per widget per cascade.
      io << " class=\""
      ...
      # must come immediately after the real children — before the sub-element
      # pseudo-nodes — in *both* documents, so a `Row`'s `:nth-child`/
      # `:nth-last-child` index counts only real children and its own siblings,
      # never the state-dependent scrollbar/track/label nodes.
      css_render_extra io
      unless structural
        # Sub-element pseudo-nodes (scrollbar, track, ...) come last. Each
        # carries a `uid::slot` writeback key routing computed style back into
        # the matching sub-`Style`, classed with the capitalized slot name so
        # `Scrollbar { ... }` / `Box Scrollbar { ... }` styles it. The host's
        # intrinsic attributes are repeated on each node — a Qt sub-control
        # shares its widget's state, so `CheckBox::indicator:checked` (lowered
        # to `Indicator[checked]`) matches.
    ...
    # matching what `#to_html` serializes for the `uid::slot` pseudo-node: the
    # writeback key, the capitalized slot class, and the host's intrinsic
    # attributes.
