...
module Crysterm
  class Screen
    ...
    #
    # Emits an `Event::Mouse` on the screen, then locates the topmost clickable
    # widget under the pointer (`#widget_at`) and, if found, emits an
    # `Event::Mouse` on it as well — and an `Event::Click` on a button press.
    def dispatch_mouse(ev : ::Tput::Mouse::Event)
      ...
      if w
        w.emit ::Crysterm::Event::Mouse.new ev
        if ev.action.down?
