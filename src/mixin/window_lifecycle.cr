module Crysterm
  module Mixin
    # The "install-on-`Attached` / uninstall-on-`Detached` / and-also-now" window
    ...
    # while still attached never emits `Detached`), and `also_now: false` to skip
    ...
    # `Detached` payload: `window?` is already nil by then, and
    ...
    module WindowLifecycle
      ...
      # the including widget. Called on `Event::Attached` and — unless wired with
      ...
      # including widget. Called on `Event::Detached`, and on `Event::Destroy`
      ...
      # `Attached` that already fired.
      private def wire_window_lifecycle(destroy = false, also_now = true) : Nil
        on(::Crysterm::Event::Attached) { on_attach_window }
        on(::Crysterm::Event::Detached) { on_detach_window }
