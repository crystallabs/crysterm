module Crysterm
  ...
  # `#interrupt` — matching emacs's "append consecutive kills" behavior. Older
  # entries are retained up to `#max` for a future yank-pop.
  #
  ...
  class KillRing
    ...
    # false — `Ctrl-K` / `Alt-D`) joins the back. Consecutive kills merge; a
    # non-kill action in between (`#interrupt`) starts a fresh entry. Empty text
    # is ignored.
