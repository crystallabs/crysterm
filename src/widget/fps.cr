...
module Crysterm
  class Widget
    ...
    #   :throughput      :throughput_avg      bytes/sec, raw integers
    #   :throughput_h    :throughput_avg_h    bytes/sec, human-readable ("3.2KiB")
    ...
    class Fps < Box
      ...
      DEFAULT_FORMAT = "R/D/FPS: %5s/%5s/%5s (%5s/%5s/%5s)  TX: %9s/s (%9s/s)  Σ: %9s"
      DEFAULT_ARGS   = %i[
        render draw fps
