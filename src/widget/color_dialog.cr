...
module Crysterm
  class Widget
    ...
    # dialog.
    #
    # It combines several editors, all kept in sync on one HSV state:
    #
    #   * a 2-D **saturation/value field** — a gradient of the current hue;
    #     click (or drag, or arrow-key) anywhere in it to set saturation (X) and
    #     value (Y); the wheel nudges the value.
    #   * a vertical **hue bar** — the full spectrum; click/drag/wheel to set hue.
    #   * a **live preview** swatch plus a **Hex / R,G,B / H,S,V** readout.
    #   * a **palette** of quick-pick named colors along the bottom.
    ...
    #
    # ```
    # dialog = Widget::ColorDialog.new parent: screen, width: 50, height: 18,
    ...
    class ColorDialog < Box
      ...
      # The quick-pick palette (named colors that always resolve as tags/specs).
      DEFAULT_COLORS = %w[
      ...
      # The quick-pick palette, in display order.
      ...
      def initialize(colors : Array(String)? = nil, **box)
        ...
        build_children
        # React to clicks/drags/wheel over the gradient field and hue bar (the
        # palette swatches and buttons are child widgets with their own handlers).
      ...
      # Sets the current color from a name or `"#rrggbb"` spec, updating all
      # editors.
      ...
      # cancel). Saves and later restores focus, and installs the modal key
      # handler (arrows move in the field, `[`/`]` change hue, Enter accepts,
      # Escape cancels).
      ...
      private def build_children
        @preview = Box.new parent: self, top: 0, left: INFO_X, width: 12, height: 3,
          style: Style.new(border: true)
        @readout = Box.new parent: self, top: 4, left: INFO_X, width: 18, height: 6,
          parse_tags: true, style: Style.new(fg: "white")

        # Palette swatches: one click sets the color.
        x = 0
        @colors.each do |name|
      ...
      # Refreshes the preview swatch + textual readout from the HSV state.
      private def refresh_ui : Nil
        ...
        if ro = @readout
          ro.set_content String.build { |io|
            io << "Hex: " << hex << '\n'
            io << "RGB: " << r << ", " << g << ", " << b << '\n'
            io << "H: " << @hue.round.to_i << "°\n"
            io << "S: " << (@saturation * 100).round.to_i << "%\n"
            io << "V: " << (@value_v * 100).round.to_i << '%'
          }
        end
        request_render
      end
      ...
      private def on_key(e : Crysterm::Event::KeyPress) : Nil
        ...
        when Tput::Key::Escape then cancel; e.accept
          ...
          case e.char
          when '[' then set_hsv @hue - 10, @saturation, @value_v; e.accept
          when ']' then set_hsv @hue + 10, @saturation, @value_v; e.accept
          end
        end
