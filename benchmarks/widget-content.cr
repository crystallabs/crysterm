...
#   * reparse_tag — same, with `{bold}`-tagged content (adds `expand_tags`).
#   * wrap_multi  — `wrap_lines` of a multi-line, word-wrapped paragraph.
#   * parse_tags  — `expand_tags` alone on a heavily-tagged line.
#
...
Benchmark.ips do |x|
  ...
  x.report("wrap_lines multi") { para.wrap_lines(para.content, 38, into: para.wrapped_lines) }

  x.report("expand_tags heavy") { tagged.expand_tags(heavy_tags) }
end
