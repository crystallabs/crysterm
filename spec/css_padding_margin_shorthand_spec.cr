...
# Focused specs for the CSS `padding` / `margin` shorthand parsers
# (`Crysterm::CSS::Properties.apply`). As with `font` / `text-decoration`, the
# interesting case is a *blank* value — an undefined `var(--x)` collapsed to ""
# before reaching the property. It must be dropped (CSS's "drop the invalid
# declaration" rule), NOT reset the box to default (which `parse_sides("")`
# -> `nil` -> `Padding.default`/`Margin.default` would otherwise do), clobbering
# a value a lower-priority cascade rule had set.
describe "CSS padding/margin shorthand" do
  ...
  it "drops a blank padding value, keeping a previously-set padding" do
    ...
    # Undefined `var()` collapses to "" (and whitespace-only) before reaching here.
