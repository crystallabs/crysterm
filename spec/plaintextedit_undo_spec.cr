...
private def type_str(pte, str : String)
  str.each_char { |c| pte._listener kp(c) }
end

describe "Widget::PlainTextEdit undo/redo" do
  it "backs onto a real TextDocument" do
    s = headless_screen(40, 6, default_quit_keys: true)
    pte = new_pte s, "hi"
  ...
  it "undoes and redoes typed text" do
    ...
    pte._listener kp('\0', ::Tput::Key::CtrlZ)
    pte.value.should eq ""

    pte._listener kp('\0', ::Tput::Key::AltZ)
    pte.value.should eq "hello"
  end
  ...
  it "coalesces consecutive typing into one undo step but breaks on deletion" do
    ...
    pte = new_pte s
    type_str pte, "abc"
    pte._listener kp('\0', ::Tput::Key::Backspace) # "ab"
