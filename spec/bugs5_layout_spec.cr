...
#
# The clip section binds `b = sp_border.top`; reusing it for the bottom-edge
# trigger is harmless with a uniform border top == bottom, but for an asymmetric
# border (`border-top-width: 1; border-bottom-width: 0`) a child sitting on the
# parent's last visible row (its `yl` == the parent's outer bottom) was compared
# against `parent.yl - 1` instead of `parent.yl`, spuriously treated as fully
# below the viewport, and dropped entirely (its `lpos` went nil). The bottom
# trigger must use a separate `bb = sp_border.bottom`.
describe "BUGS5 coords bottom clip uses the parent's bottom border (fix #1)" do
  it "renders a child on the last visible row of a bottom-border-0 parent" do
    ...
    # The child on the last visible row is NOT dropped.
    cl = child.lpos
...
#
# The vertical clip triggers at `parent.yi + border.top` / `parent.yl -
...
# (whereas the symmetric `top: -1` WAS clipped). The horizontal triggers must
# include `bl`/`br` so they clip at the inner border edge like the vertical.
describe "BUGS5 coords horizontal clip uses the parent's left/right border (fix #2)" do
  it "clips a child at left:-1 to the inner edge of a left-bordered parent" do
    ...
    # exactly `parent.xi`. An outer-edge trigger would leave it there (painting
    # over the border); the clip must shift it to the inner content edge.
    ...
    cl.should_not be_nil
    cl = cl.not_nil!
    # The child starts at the inner border edge, not on the border.
