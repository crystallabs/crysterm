...
#
# * W8 — `_minimal_children_rectangle` (the shrink-to-content measurer) skips
...
#   stretched slot, so a `resizable` widget locked at its parent's full size
#   after the first frame instead of tracking its real children.
# * W19 — `Widget.dimension`'s +/- offset parser (and the `center±N` path in
...
describe "BUGS13 W8: shrink-to-content skips layout_excluded chrome" do
  it "a full-slot excluded child does not balloon the shrink size across renders" do
    s = layout_screen
    shrink = Widget::Box.new parent: s, top: 0, left: 0, resizable: true
    ...
    ref = Widget::Box.new parent: s, top: 5, left: 0, resizable: true
...
describe "BUGS13 W19: malformed +/- offsets read as 0" do
  it "Widget.dimension accumulates only digit offsets" do
    # Clean offsets still apply...
    Widget.dimension("50%+5", 100).should eq 55
    Widget.dimension("50%-3", 100).should eq 47
    # ...malformed ones read as 0 instead of folding arbitrary bytes.
    Widget.dimension("50%+1.5", 100).should eq 50 # pre-fix: 135 (dim 270)
    Widget.dimension("50%+5x", 100).should eq 50
    Widget.dimension("50%+", 100).should eq 50
    # The space also breaks the percentage parse, so the whole expression is
    # malformed → 0 (pre-fix: -155).
    Widget.dimension("50% + 5", 100).should eq 0
  end
