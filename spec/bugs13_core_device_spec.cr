...
describe "BUGS13 C6: switch_terminal carries pin state" do
  ...
  it "does not pin an unpinned window's axes (still tracks resizes)" do
    ...
    w2 = w.switch_terminal "xterm"
    begin
      # Passing the current size as plain Int32s would pin both axes:
      # `refresh_size`/`adopt_terminal_size` then no-op forever and the
      # replacement window freezes at the moment-of-switch size.
