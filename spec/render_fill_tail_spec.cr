...
describe "content-exhausted fill tail" do
  it "fills the tail with the fill char and leaves content intact" do
    ...
    w.style.fill_char = '.'
    s.repaint
    String.build { |io| 5.times { |x| io << s.cell_rows[0][x].char } }.should eq "hello"
    s.cell_rows[0][10].char.should eq '.'
    s.cell_rows[3][7].char.should eq '.' # a row fully past the content
  end
  ...
  it "keeps a dangling SGR attribute across the filled tail" do
    ...
    s.repaint
    red = Attr.bg(s.cell_rows[0][0].attr)
    red.should_not eq Attr.bg(Window::DEFAULT_ATTR)
    # The dangling attr keeps painting the fill cells, rows later.
    Attr.bg(s.cell_rows[0][10].attr).should eq red
    Attr.bg(s.cell_rows[2][5].attr).should eq red
  end
  ...
  it "paints the valign gap and the padding bands" do
    ...
    s.repaint
    s.cell_rows[0][5].char.should eq '.' # top padding band
    s.cell_rows[2][5].char.should eq '.' # valign gap row (interior)
    s.cell_rows[1][0].char.should eq '.' # left padding band
    s.cell_rows[4][1].char.should eq 'x' # content landed at the bottom interior row
  end
  ...
  it "still blends per cell for an alpha widget's tail" do
    ...
    mixed = Attr.bg s.cell_rows[2][10].attr
  ...
  it "lays a wide fill char one per cell under full_unicode (no continuation claim)" do
    ...
    w.style.fill_char = '好'
    s.repaint
    s.cell_rows[1][0].char.should eq '好'
    s.cell_rows[1][1].char.should eq '好'
    s.cell_rows[1][5].char.should eq '好'
  end
end
