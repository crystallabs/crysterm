...
# real name, not by reverse-engineering the decorated row text (which carries
# `{...}` color tags plus a `/`/`@` suffix):
#   * a name containing `{...}` would be mangled by `clean_tags` (strips it as
#     a style tag), and
