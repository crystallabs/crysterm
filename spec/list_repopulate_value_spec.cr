...
# `ItemView#selekt` latches `@_list_initialized` so the first selection of index
...
# `#value` stays stale and no `SelectItem` fires.
describe "ItemView#selekt value after re-populating an emptied list" do
  it "refreshes #value when the first row is appended to an emptied list" do
    ...
    # Second removal takes it through the empty-selection branch of `#selekt`.
