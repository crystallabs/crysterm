...
# not leave `style.alpha` pinned at 0.0, or a *later* plain `#show` would repaint
...
describe "Widget#fade_out" do
  it "hides the widget and clears the residual alpha so a later #show is opaque" do
    s = sized_screen 10, 3
    ...
    # No residual transparency left behind by the tween's final (alpha == 0) tick.
