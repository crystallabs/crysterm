...
# but dropped the trailing `update!`, so a programmatic state
...
# disagreed.
#
# `update!` both `damage_mark_dirty`s the widget (recording its
# top-level ancestor in the screen's pending dirty-roots set under
# `DamageTracking`) and calls `Window#update`. In a headless spec the render
# fiber never runs, so `Window#update` only rings the doorbell — the
...
# `@damage_dirty_roots`, so only `update!` lands the checkbox in the
...
describe "CheckBox programmatic state changes schedule a repaint" do
  it "schedules a repaint when checked programmatically" do
    ...
    repaint_scheduled?(s, cb).should be_true # `check`'s `update!` marked it
  end
