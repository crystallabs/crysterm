...
# Same defect as BUGS15 #20, which covered only the `each_arrangeable` engines.
describe "BUGS16 16: Flow engines do not arrange border-label/scrollbar chrome" do
  it "keeps a Wrap-container's border label on the border row, not in a slot" do
    ...
    # The label stays pinned to the border row (top == -itop), not arranged
    # into interior slot 0 at top 0 / left 0.
  ...
  it "does not let a border label inflate a UniformGrid's column width" do
    ...
    # A label awidth folded into the uniform column width would collapse the
    # grid to one column and wrap c2 to a second row; both 5-wide children
    # must share the first row.
