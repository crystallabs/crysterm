...
# before reaching the property; per CSS's "drop the invalid declaration" rule
# it must be ignored, leaving any previously-cascaded size intact. The old
# unguarded form ran `parse_background_size("")`, which falls through to its
# `Cover` default, silently resetting an already-set size. The sibling
# `background-image` longhand already guards this case; size must too.
describe "CSS background-size blank longhand" do
  it "drops a blank value, keeping a previously-set size" do
    ...
    # A collapsed undefined `var()` reaches here as "" — must not reset to `Cover`.
