...
# would keep ticking alongside the new one and both would fire `stop_handler`. The
...
describe Crysterm::FrameClock do
  it "does not leave a superseded fiber running after a same-instance stop+start" do
    ...
    # Exactly one `stop_handler`. Without the generation guard the orphaned fiber
    # also finalizes -> 2.
    stops.should eq 1
  end

  it "does not double-fire stop_handler when reduced motion is enabled between a stop and a restart" do
    ...
    # causing a duplicate `stop_handler`.
    ...
    anim.start # reduced-motion path: completes synchronously, fires stop_handler once
