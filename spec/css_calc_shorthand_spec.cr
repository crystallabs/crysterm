...
# (`padding`/`margin`/`border-width`). CSS requires whitespace around `+`/`-`
# in `calc()` (`calc(2em + 1em)`; `calc(2em+1em)` is invalid), so the shorthand
# parsers must keep a space-bearing `calc()` as one token. They split with
# `split_top_level` (paren-aware), like the color shorthands; a plain
# `value.split` would shred it into `calc(2em`/`+`/`1em)` and mis-apply the
# declaration.
#
# `em` is used throughout: divisor `1.0`, isotropic (not scaled by cell aspect
# ratio), so `1em == 1 cell` on both axes.
