...
describe "drag-and-drop" do
  describe "reposition (self-move) via mouse" do
    it "moves the widget with the pointer, keeping the grab offset" do
      ...
      mouse_down s, 12, 6 # grab at offset (2, 1) within the box
      mouse_move s, 14, 8 # promotes arm -> drag, then this is the first motion
    ...
    it "does not start a drag when the widget accepts the press itself" do
      ...
      mouse_down s, 12, 6 # press over the draggable box; the widget accepts it
      mouse_move s, 14, 8 # a motion that would otherwise promote the arm into a drag
    ...
    it "moves a nested draggable widget relative to its parent's content origin" do
      ...
      mouse_down s, 27, 11 # grab at absolute (27, 11) -> offset (2, 1) within the box
      mouse_move s, 30, 14 # promote arm -> drag, first motion to absolute (30, 14)
    ...
    it "clamps a nested widget within the parent's content area, not past its border" do
      ...
      mouse_down s, panel.aleft + 1 + 2, panel.atop + 1 + 2 # grab at offset (0,0) within the box
      mouse_move s, panel.aleft + 1 + 3, panel.atop + 1 + 3 # promote arm -> drag
      mouse_move s, 999, 999                                # yank far past the bottom-right
    ...
    it "clamps the widget within the screen bounds" do
      s = headless_screen(default_quit_keys: true) # 80x24
      box = Widget::Box.new parent: s, left: 2, top: 2, width: 8, height: 4, draggable: true

      mouse_down s, 2, 2
      mouse_move s, 1, 1     # promote
      mouse_move s, -50, -50 # far past the top-left corner
    ...
    it "moves a top-level widget relative to the screen's padded content origin" do
      ...
      mouse_down s, 9, 6  # grab at offset (2, 1) within the box
      mouse_move s, 12, 9 # promote arm -> drag, first motion
    ...
    it "clamps a top-level widget within the screen's padded content area" do
      ...
      mouse_down s, 3, 2     # grab at offset (0, 0) within the box (its content corner)
      mouse_move s, 4, 3     # promote
      mouse_move s, 999, 999 # yank far past the bottom-right
  ...
  describe "data transfer via mouse" do
    it "negotiates a drop: source advertises payload, target accepts, Drop fires" do
      ...
      mouse_down s, 1, 1
      mouse_move s, 2, 1 # promote the drag (source stays put, reposition off)
      source.left.should eq(0)
      mouse_move s, 44, 1 # drag over the target
      mouse_up s, 44, 1
    ...
    it "emits DragEnter/DragLeave as the pointer crosses a target" do
      ...
      mouse_down s, 1, 1
      mouse_move s, 2, 1  # promote, not over target
      mouse_move s, 44, 1 # enter target
      mouse_move s, 45, 2 # still over target
      mouse_move s, 70, 1 # leave target
      mouse_up s, 70, 1
  ...
  describe "two-click mouse fallback" do
    it "lifts on the first click and drops on the second (no motion needed)" do
      ...
      mouse_down s, 1, 1 # first click: lift
      s.drag_session.should_not be_nil
      mouse_down s, 44, 1 # second click over target: drop
