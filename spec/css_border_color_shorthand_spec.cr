...
# standard CSS fill-ins (1 → all sides, 2 → vertical/horizontal, 3 → top/
# horizontal/bottom). The renderer paints each edge from its per-side color
# (`Border#top_fg`/`#right_fg`/…, which fall back to the whole-border `#fg`), so a
# multi-value `border-color` must populate those per-side slots — the direct
# analog of the multi-value `border-width` shorthand.
#
# The old form passed the whole multi-token value to `with_color`, which resolved
# e.g. `"red green blue yellow"` to the `-1` unknown sentinel (terminal default)
# and cleared every side, silently dropping all four colors.
