...
# Behavior lock for `SGR.to_attr` (SGR sequence -> packed Int64 attr).
...
describe "SGR.to_attr" do
  ...
  it "resets to default on \\e[0m and \\e[m (empty params)" do
    bolded = Crysterm::SGR.to_attr("\e[1m", dfl, dfl)
    (Attr.flags(bolded) & Attr::BOLD).should_not eq 0

    Crysterm::SGR.to_attr("\e[0m", bolded, dfl).should eq dfl
    Crysterm::SGR.to_attr("\e[m", bolded, dfl).should eq dfl # empty => 0 => reset
  end
  ...
  it "sets and clears individual style flags" do
    ...
    Attr.flags(Crysterm::SGR.to_attr("\e[22m", bold, dfl)).should eq Attr.flags(dfl)
  end
  ...
  it "clears only the respective flag on a partial reset, leaving others set" do
    ...
    after = Crysterm::SGR.to_attr("\e[24m", multi, dfl)
    ...
    # The other partial resets likewise clear only their own bit.
    {
      ...
      res = Crysterm::SGR.to_attr(code, all, dfl)
  ...
  it "applies 39/49 default fg/bg" do
    ...
    cleared_fg = Crysterm::SGR.to_attr("\e[39m", colored, dfl)
    Attr.unpack_color(Attr.fg(cleared_fg)).should eq -1
    cleared_bg = Crysterm::SGR.to_attr("\e[49m", colored, dfl)
