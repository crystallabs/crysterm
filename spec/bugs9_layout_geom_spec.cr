...
# A resizable (shrink-to-content) box holding one fixed child, anchored to
...
private def shrink_box_rect(anchor : Symbol, pad : Crysterm::Padding, child_w = 6, child_h = 3)
  ...
  sh =
    case anchor
    when :left  then Widget::Box.new parent: s, top: 2, left: 2, resizable: true, style: st
    when :right then Widget::Box.new parent: s, top: 2, right: 2, resizable: true, style: st
    when :top   then Widget::Box.new parent: s, left: 2, top: 2, resizable: true, style: st
    else             Widget::Box.new parent: s, left: 2, bottom: 2, resizable: true, style: st
    end
...
# `content + iwidth`/`iheight`; the far-anchored branch must subtract the *far*
...
describe "BUGS9 shrink-to-content far-anchor uses the far inset (fix #1)" do
  it "sizes a right-anchored shrink box to content + iwidth under asymmetric padding" do
  ...
  it "sizes a bottom-anchored shrink box to content + iheight under asymmetric padding" do
    ...
    th.should eq(6)  # content 3 + iheight 3
  ...
  it "leaves the symmetric-inset case unchanged (both anchors equal)" do
    ...
    (lw[1] - lw[0]).should eq(10) # content 6 + iwidth 4
    (rw[1] - rw[0]).should eq(10)
    th = shrink_box_rect(:top, pad); bh = shrink_box_rect(:bottom, pad)
    (th[3] - th[2]).should eq(7) # content 3 + iheight 4
