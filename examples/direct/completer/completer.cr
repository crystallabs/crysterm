...
# Prefix typed, completion expected, pretend result line — cycled by the demo.
STEPS = [
  {"de", "deploy", "✓ deploy: 3 targets updated"},
  {"sp", "spec", "✓ spec: 128 examples, 0 failures"},
  {"fo", "format", "✓ format: 42 files, no changes"},
]
...
s.every(0.3.seconds) do
  prefix, _completion, result = STEPS[step % STEPS.size]
  case tick % 12
  when 0 then cmd.focus
  when 1 then press.call prefix[0], nil
  when 3 then press.call prefix[1], nil
  when 5 then press.call '\0', ::Tput::Key::Down # highlight the top match
  when 8 then press.call '\r', ::Tput::Key::Enter
  when 10
    history.content = "{#8a94a6-fg}❯ #{cmd.value}\n{/}{#98c379-fg}  #{result}{/}"
    cmd.value = ""
    step += 1
  end
  tick += 1
