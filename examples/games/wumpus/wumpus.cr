...
# Hunt the Wumpus — a port of Gregory Yob's 1973 classic (see `help` below for
# the rules and the flag/preset system).
#
# The program is split in two files. `ui.cr` is the reusable part: `WumpusUI`
# builds the teletype — the scrolling transcript, the input line and the
# corner scoreboard — and keeps keyboard focus on the input. This file is the
# game: the cave, the commands, the flags and every line of text. To build
# your own text adventure on the same chrome, keep `ui.cr` and replace this
# file.
class Wumpus
  private def help
    say "
...
{bold}Commands in current config (type, then press Enter):{/}"
  ...
  def initialize(@opt : Hash(String, Bool))
    ...
    @ui = WumpusUI.new(@window)
    @transcript = @ui.transcript
    @input = @ui.input
    # Shown only when "score" is on (see update_score).
    @scorebox = @ui.scorebox
